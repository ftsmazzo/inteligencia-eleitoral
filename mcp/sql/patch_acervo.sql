...
BEGIN
  ...
  FROM (
    SELECT
      ...
      + CASE
          WHEN api.acervo_norm(c.texto) LIKE '%' || api.acervo_norm(v_q) || '%' THEN 0.05
          WHEN api.acervo_norm(d.titulo) LIKE '%' || api.acervo_norm(v_q) || '%' THEN 0.02
          ELSE 0
        END AS rank
    FROM acervo.chunk c
    ...
    WHERE d.ativo IS TRUE
      ...
      AND (
        to_tsvector('portuguese', c.texto) @@ plainto_tsquery('portuguese', v_q)
        OR api.acervo_norm(c.texto) LIKE '%' || api.acervo_norm(v_q) || '%'
        OR api.acervo_norm(d.titulo) LIKE '%' || api.acervo_norm(v_q) || '%'
      )
    ORDER BY rank DESC
    LIMIT v_lim
  ) t;
