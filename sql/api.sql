...
AS $$
  SELECT jsonb_build_object(
    ...
    'pacotes', jsonb_build_array(
      ...
      jsonb_build_object('pacote','municipio','nota','resolve nome→cod_ibge (UF opcional); use antes de votacao/prefeito/vereador'),
      jsonb_build_object('pacote','nominata','anos','2014–2026','nota','2026 sem resultado de urna; cargo geral=UF; municipal=cod_ibge'),
      jsonb_build_object('pacote','votacao','anos','2014,2016,2018,2020,2022,2024','nota','exige ano+cargo e (uf ou cod_ibge ou nacional=true); cidade ok para qualquer cargo (votos no município)'),
      ...
      jsonb_build_object('pacote','rede_social','anos','2020,2022,2024,2026','nota','URLs/handles TSE; exige ano+sq_candidato'),
      jsonb_build_object('pacote','complementar','anos','2018–2026','nota','campos extras TSE (sem CPF); exige ano+sq_candidato'),
      jsonb_build_object('pacote','receita','anos','2014–2026','nota','prestação; exige ano e (sq_candidato ou uf); sem CPF'),
      jsonb_build_object('pacote','despesa','anos','2014–2026','nota','prestação; exige ano e (sq_candidato ou uf); categoria opcional (heurística); sem CPF'),
      jsonb_build_object('pacote','contas_resumo','anos','2014–2026','nota','totais receita/despesa + categorias + custo/voto; exige ano e (sq_candidato ou uf); cargo recomendado'),
...
REVOKE ALL ON FUNCTION api.rede_social(smallint, bigint, integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION api.complementar(smallint, bigint) FROM PUBLIC;
REVOKE ALL ON FUNCTION api.receita(smallint, bigint, text, text, text, integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION api.despesa(smallint, bigint, text, text, text, integer, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION api.contas_resumo(smallint, text, text, text, bigint, integer, boolean) FROM PUBLIC;
REVOKE ALL ON FUNCTION api._categoria_despesa(text) FROM PUBLIC;
...
-- municipio (tamb�m em patch_municipio_api.sql)
