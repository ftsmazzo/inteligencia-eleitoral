...
`catalogo`, `votacao`, `comparecimento`, `nominata`, `eleitorado`.  
`votacao` exige `ano` + `cargo` + (`uf` ou `cod_ibge`), salvo totais nacionais explícitos. Sem SQL livre. Fora do recorte: texto seco da spec.
